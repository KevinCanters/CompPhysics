...
def compute_scf(geoint, guess, nstep=20, threshold=1e-6):
    ...
    while True:
        ...
        dm_alpha = np.dot(evecs_alpha[:, :nalpha], evecs_alpha[:, :nalpha].T)
        dm_alpha[16,:] *= 0.5
        dm_alpha[15,:] *= 0.5
        dm_beta = np.dot(evecs_beta[:, :nbeta], evecs_beta[:, :nbeta].T)
        lambda_var = 0.35
        dm_total = (1-lambda_var)*dm_total + lambda_var*(0.5*dm_alpha + 0.5*dm_beta)
    print('------------------------------------------------------')
