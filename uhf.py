#!/usr/bin/env python3
"""Compute UHF energy for a given data file. Run as follows:

    ./uhf.py --help

or
    ./uhf.py fn_geoint fn_scfopt nalpha nbeta [fn_guess]
"""
...
def core_hamiltonian_guess(geoint, nalpha, nbeta):
    ...
    guess['dm_alpha'] = np.dot(evecs[:, :nalpha], evecs[:, :nalpha].T)
    guess['dm_beta'] = np.dot(evecs[:, :nbeta], evecs[:, :nbeta].T)
    return guess
    

def compute_scf(geoint, guess, nstep=20, threshold=1e-6):
    """Basic UHF SCF solver.
    ...
    while True:
        ...
        energy = sum([
            ...
            0.5*expval(coulomb, dm_total),
            -0.5*expval(exchange_alpha, dm_alpha),
            -0.5*expval(exchange_beta, dm_beta),
        ])

        # Compute the Fock matrices
        fock_alpha = coreham + coulomb - exchange_alpha
        fock_beta = coreham + coulomb - exchange_beta
        ...
        ###START OF MODIFICATION (adding lambda to mix the density matrices)###
        lambda_var = 0.35
        dm_alpha = (1-lambda_var)*dm_alpha + lambda_var*np.dot(evecs_alpha[:, :nalpha], evecs_alpha[:, :nalpha].T) #Adjust for D = (1-l)*D_old + l* D_new
        dm_beta = (1-lambda_var)*dm_beta + lambda_var*np.dot(evecs_beta[:, :nbeta], evecs_beta[:, :nbeta].T)
        dm_total = dm_alpha + dm_beta
        ###END OF MODIFICATION (adding lambda to mix the density matrices)###
    print('------------------------------------------------------')
